...
describe("gridHintGeometry", () => {
  ...
  it("sizes arrowhead with classic proportions (1.8× shaft, depth = width)", () => {
    expect(ARROW_HEAD_WIDTH_TO_SHAFT_RATIO).toBe(1.8);
    expect(ARROW_HEAD_LENGTH_TO_WIDTH_RATIO).toBe(1);
    expect(arrowHeadDimensions(40)).toEqual({ headWidth: 72, headLength: 72 });
  });
  ...
  it("points arrow tip at head with head base behind tip along travel", () => {
    ...
    expect(path).toContain("L 64 68");
    expect(path).toContain("L 64 32");
  });
